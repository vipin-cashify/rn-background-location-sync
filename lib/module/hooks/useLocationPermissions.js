...
import { LocationPermissionStatus, NotificationPermissionStatus } from "../types/index.js";
import BackgroundLocationModule from "../NativeBackgroundLocation.js";
import { resolveRationale } from "../utils/index.js";
 ...
 */
function warnNotificationDenied(status) {
  if (status !== 'granted' && __DEV__ && !notificationWarned) {
    console.warn('[BackgroundLocation] Notification permission was denied. Geofence visual notifications will not appear. Background tracking is unaffected.');
 ...
 */
function toLocationPermissionState(result) {
  const status = mapNativeStatus(result.status);
  return {
    hasPermission: status === LocationPermissionStatus.GRANTED || status === LocationPermissionStatus.WHEN_IN_USE,
    status,
 ...
 */
function toNotificationPermissionState(status) {
  let mappedStatus;
  switch (status) {
    case 'granted':
...
export function useLocationPermissions() {
  ...
  const checkPermissions = useCallback(async () => {
    if (Platform.OS === 'ios') {
      try {
        const [locationResult, notificationStatus] = await Promise.all([BackgroundLocationModule.checkLocationPermission(), BackgroundLocationModule.checkNotificationPermission()]);
    ...
    try {
      ...
      const locationGranted = fineLocation && coarseLocation && backgroundLocation;
      const location = {
        hasPermission: locationGranted,
        status: locationGranted ? LocationPermissionStatus.GRANTED : LocationPermissionStatus.DENIED,
        canRequestAgain: true
      };
  ...
  const requestPermissions = useCallback(async options => {
    if (Platform.OS === 'ios') {
      ...
      try {
        ...
        const locationResult = await BackgroundLocationModule.requestLocationPermission(false);
        const location = toLocationPermissionState(locationResult);

        // Step 2: Request notification permission (non-blocking)
        const notificationStatus = await BackgroundLocationModule.requestNotificationPermission();
        const notification = toNotificationPermissionState(notificationStatus);
    ...
    try {
      ...
      if (Platform.Version >= 29) {
        const backgroundResult = await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.ACCESS_BACKGROUND_LOCATION, resolveRationale(options?.backgroundRationale));
