...
import { extractDefinedProperties } from "../utils/objectUtils.js";
 ...
 * ```
 */
export function useLocationUpdates(options = {}) {
  const {
  ...
  const [tripId, setTripId] = useState(providedTripId || null);
  ...
  const clearLocations = useCallback(async () => {
    ...
    try {
      ...
      const clearError_instance = err instanceof Error ? err : new Error('Failed to clear locations');
   ...
   */
  const loadExistingLocations = useCallback(async loadTripId => {
    if (!isNativeModuleAvailable()) {
      return;
    }
    setIsLoading(true);
    ...
    try {
      ...
      const loadError = err instanceof Error ? err : new Error('Failed to load existing locations');
      setError(loadError);
  ...
  useEffect(() => {
    ...
    const subscription = eventEmitter.addListener('onLocationUpdate', event => {
      ...
      if (!tripId || locationEvent.tripId === tripId) {
        ...
        const newLocation = extractDefinedProperties(locationEvent);
        ...
        if (onLocationUpdateRef.current) {
          const now = Date.now();
          if (!onUpdateInterval || now - lastCallbackTimeRef.current >= onUpdateInterval) {
  ...
  useEffect(() => {
    ...
    const eventEmitter = new NativeEventEmitter(BackgroundLocationModule);
    const warningSubscription = eventEmitter.addListener('onLocationWarning', event => {
  ...
  useEffect(() => {
    ...
    const eventEmitter = new NativeEventEmitter(BackgroundLocationModule);
    const actionSubscription = eventEmitter.addListener('onNotificationAction', event => {
  ...
  useEffect(() => {
    const subscription = AppState.addEventListener('change', nextAppState => {
      ...
      if (nextAppState === 'active' && (previousState === 'background' || previousState === 'inactive') && tripId && autoLoad && !wasClearedRef.current) {
  ...
  return useMemo(() => ({
    ...
    clearLocations,
    refreshLocations
  }), [tripId, isTracking, locations, lastLocation, lastWarning, isLoading, error, clearError, clearLocations, refreshLocations]);
}
//# sourceMappingURL=useLocationUpdates.js.map