...
import { toTrackingOptionsSpec } from "../utils/index.js";
...
export function useBackgroundLocation(options = {}) {
    ...
    const checkStatus = async () => {
      ...
      try {
        ...
        if (status.tripId) {
          ...
          const locs = await BackgroundLocationModule.getLocations(status.tripId);
          setLocations(locs);
        }
      } catch (err) {
   ...
   * Start tracking
   */
  const startTracking = useCallback(async (customTripId, trackingOptionsOverride) => {
    if (!isNativeModuleAvailable()) {
      const simulatedId = customTripId || `simulator-trip-${Date.now()}`;
      setTripId(simulatedId);
      setIsTracking(true);
      console.warn('BackgroundLocation not available - using simulated trip ID');
      return simulatedId;
    }
    setIsLoading(true);
    clearError();
    try {
      ...
      const specOptions = effectiveOptions ? toTrackingOptionsSpec(effectiveOptions) : undefined;
      const effectiveTripId = await BackgroundLocationModule.startTracking(customTripId, specOptions);
      ...
      const trackingError = err instanceof Error ? err : new Error('Failed to start tracking');
  ...
  const stopTracking = useCallback(async () => {
    ...
    try {
      ...
      const stopError = err instanceof Error ? err : new Error('Failed to stop tracking');
      setError(stopError);
  ...
  const refreshLocations = useCallback(async () => {
    ...
    try {
      ...
      const locError = err instanceof Error ? err : new Error('Failed to get locations');
      setError(locError);
  ...
  const clearCurrentTrip = useCallback(async () => {
    ...
    try {
      ...
      const tripError = err instanceof Error ? err : new Error('Failed to clear trip');
      setError(tripError);
  ...
  return useMemo(() => ({
    ...
    clearCurrentTrip,
    clearError
  }), [tripId, isTracking, locations, isLoading, error, startTracking, stopTracking, refreshLocations, clearCurrentTrip, clearError]);
}
