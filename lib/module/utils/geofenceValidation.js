 ...
 * @internal
 */
export function validateGeofenceRegion(region) {
  ...
  if (region.latitude < -90 || region.latitude > 90) {
    throw new Error(`[BackgroundLocation] Invalid latitude: ${region.latitude}. Must be between -90 and 90.`);
  }
  if (region.longitude < -180 || region.longitude > 180) {
    throw new Error(`[BackgroundLocation] Invalid longitude: ${region.longitude}. Must be between -180 and 180.`);
  }
  if (region.radius < 100) {
    throw new Error(`[BackgroundLocation] Invalid radius: ${region.radius}. Minimum is 100 meters.`);
  }
  if (region.loiteringDelay != null && region.loiteringDelay < 0) {
    throw new Error(`[BackgroundLocation] Invalid loiteringDelay: ${region.loiteringDelay}. Must be non-negative.`);
  }
}
//# sourceMappingURL=geofenceValidation.js.map