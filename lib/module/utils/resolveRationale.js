 ...
 * @internal
 */
export function resolveRationale(override) {
  const title = typeof override?.title === 'string' && override.title.trim().length > 0 ? override.title.trim() : DEFAULT_BACKGROUND_LOCATION_RATIONALE.title;
  const message = typeof override?.message === 'string' && override.message.trim().length > 0 ? override.message.trim() : DEFAULT_BACKGROUND_LOCATION_RATIONALE.message;
  const buttonPositive = typeof override?.buttonPositive === 'string' && override.buttonPositive.trim().length > 0 ? override.buttonPositive.trim() : DEFAULT_BACKGROUND_LOCATION_RATIONALE.buttonPositive;
  const buttonNegative = typeof override?.buttonNegative === 'string' && override.buttonNegative.trim().length > 0 ? override.buttonNegative.trim() : DEFAULT_BACKGROUND_LOCATION_RATIONALE.buttonNegative;
  const buttonNeutral = typeof override?.buttonNeutral === 'string' && override.buttonNeutral.trim().length > 0 ? override.buttonNeutral.trim() : DEFAULT_BACKGROUND_LOCATION_RATIONALE.buttonNeutral;
  return {
    title,
    message,
    buttonPositive,
...
//# sourceMappingURL=resolveRationale.js.map